...
- **Array sizes off-by-one in stack allocations**: Ghidra may report an array as `T arr[N]` but the actual stack allocation (`SUB ESP, X`) has room for N+1 entries. This happens when code writes `arr + 1` with N entries (e.g., `GetPaletteEntries(..., 256, arr + 1)` on a `[256]` array). The write goes to indices 1–N, and index N is past the declared array. **Always check `SUB ESP` in the ASM** to verify the real allocation size. In `TScreenPanel::set_focus`, Ghidra shows `color_table[256]` but `SUB ESP,0x408` = 1032 bytes = space for 257 entries. Using `[256]` causes a 4-byte stack buffer overflow that silently corrupts the return address.
- **PtrSpanCopy / memcpy direction (this vs param)**: Ghidra's decompiler may not clearly indicate which operand is source vs destination in copy functions. The ASM `REP MOVSB` copies ESI→EDI; check register setup to confirm `this` = source, param = destination (or vice versa). Getting it backwards produces invisible output (copies FROM the blank destination INTO the populated source).
