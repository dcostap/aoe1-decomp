...
if !CHANGED_COUNT! GTR 0 (
    ...
    powershell -NoProfile -Command "$out=@(); foreach($s in ($env:CHANGED_SOURCES -split ' ')) { if($s -and $s.Length -gt 0) { $out += ([char]34 + $s + [char]34) } }; Set-Content -LiteralPath '!CHANGED_RSP!' -Value $out -Encoding ASCII" >nul
