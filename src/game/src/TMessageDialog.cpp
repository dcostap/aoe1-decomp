...
long TMessageDialog::setup(TPanel* parent, char* info_file, long info_id, long wid, long hgt, uchar dlg_type_in, int text_id, long button_wid, long button_hgt) {
    // Fully verified. Source of truth: dlg_msg.cpp.decomp @ 0x0043FFF0
    char str[256];
    str[0] = '\0';
    this->get_string(text_id, str, sizeof(str));
    return this->setup(parent, info_file, info_id, wid, hgt, dlg_type_in, str, button_wid, button_hgt);
}

long TMessageDialog::setup(TPanel* parent, char* info_file, long info_id, long wid, long hgt, uchar dlg_type_in, char* text_in, long button_wid, long button_hgt) {
    ...
    if (!TDialogPanel::setup(area, parent, wid, hgt, info_file, info_id, 1)) {
        return 0;
    }

    this->dlg_type = dlg_type_in;

    const long text_w = this->pnl_wid - 3;
    const long text_h = (this->pnl_hgt - button_hgt) - 6;
    if (!this->create_text((TPanel*)this, &this->text, text_in, 3, 3, text_w, text_h, 0xB, 1, 1, 1)) {
        return 0;
    }
    ...
    long spacing = this->pnl_wid - num_buttons * button_wid;
    if (spacing < 1) {
        spacing = 0;
    } else {
    ...
    const long btn_y = (this->pnl_hgt - button_hgt) - 7;
    long x = spacing;
    for (int i = 0; i < num_buttons; ++i) {
        if (!this->create_button((TPanel*)this, &this->button[i], (char*)"", (char*)"", x, btn_y, button_wid, button_hgt, 0xB, 0, 0)) {
            return 0;
        }
        x += spacing + button_wid;
    }
    ...
    case '\0':
        // OK
        this->button[0]->set_text((short)0, (long)0xFA1);
        break;
    case '\x01':
        // OK/Cancel
        this->button[0]->set_text((short)0, (long)0xFA1);
        this->button[1]->set_text((short)0, (long)0xFA2);
        esc_button = 1;
        break;
    case '\x02':
        // Yes/No
        this->button[0]->set_text((short)0, (long)0xFA3);
        this->button[1]->set_text((short)0, (long)0xFA4);
        esc_button = 1;
        break;
    case '\x03':
        // Retry/Abort
        this->button[0]->set_text((short)0, (long)0xFA5);
        this->button[1]->set_text((short)0, (long)0xFA7);
        esc_button = 1;
        break;
    case '\x04':
        // Yes/No/Cancel
        this->button[0]->set_text((short)0, (long)0xFA3);
        this->button[1]->set_text((short)0, (long)0xFA4);
        this->button[2]->set_text((short)0, (long)0xFA2);
        esc_button = 2;
        break;
    default:
        break;
    ...
    if (esc_button >= 0 && this->button[esc_button]) {
        this->button[esc_button]->hotkey = 0x1B;
        this->button[esc_button]->hotkey_shift = 0;
    }
...
long TMessageDialog::action(TPanel* param_1, long param_2, ulong param_3, ulong param_4) {
    ...
    if (this->parent_panel && param_2 == 1) {
...
long TMessageDialog::setup(TDrawArea* param_1, TPanel* param_2, long param_3, long param_4, long param_5, long param_6, uchar param_7) { return TDialogPanel::setup(param_1, param_2, param_3, param_4, param_5, param_6, param_7); }
...
long TMessageDialog::key_down_action(long param_1, short param_2, int param_3, int param_4, int param_5) { (void)param_1; (void)param_2; (void)param_3; (void)param_4; (void)param_5; return 0; }
