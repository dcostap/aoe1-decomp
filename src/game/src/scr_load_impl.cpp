...
long TribeLoadSavedGameScreen::action(TPanel* param_1, long param_2, ulong param_3, ulong param_4) {
    if (param_1) {
        ...
        if ((TButtonPanel*)param_1 == this->cancelButton && param_2 == 1) {
            ...
            // NOTE: The original returns to "Game Screen" when present and unpauses as needed.
            // Our current panel manager does not expose named-panel lookup yet, so this fallback always
            // returns to "Single Player Menu" in the SP-only flow.
