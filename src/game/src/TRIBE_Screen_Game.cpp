...
void TRIBE_Screen_Game::handle_game_update() {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x00496800
    if (rge_base_game == nullptr) {
        return;
    }
...
void TRIBE_Screen_Game::player_changed(int old_player, int new_player) {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x00498A50
...
void TRIBE_Screen_Game::handleChatReceived(int from_player) {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x00497330
    char chat_msg[256];
    char* src = (chat != nullptr) ? ((TChat*)chat)->GetChatMsg(from_player) : nullptr;
    if (src == nullptr) {
        src = (char*)"";
    }
    ...
    if (panel != nullptr) {
        panel->show_message(TMessagePanel::ChatMessage, chat_msg, 0xFF, 0, nullptr, 0, 0);
    }
...
void TRIBE_Screen_Game::display_system_message(char* text) {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x004973F0
    if (this->runtime.chat_line < 0 || this->runtime.chat_line > 7) {
        this->runtime.chat_line = 0;
    }
    ...
    TMessagePanel* panel = this->runtime.chat_panel[this->runtime.chat_line];
    if (panel != nullptr) {
        panel->show_message(TMessagePanel::ChatMessage, text, 0xFF, 0, nullptr, 0, 0);
    }
...
void TRIBE_Screen_Game::setup_buttons() {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x004996C0.
...
void TRIBE_Screen_Game::command_score(int enabled) {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049C650.
...
void TRIBE_Screen_Game::reset_score_display() {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049B080.
...
void TRIBE_Screen_Game::reset_clocks() {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049ADA0.
    if (this->runtime.world == nullptr || this->runtime.world->players == nullptr) {
        return;
    }
...
void TRIBE_Screen_Game::reset_buttons() {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x004996B0.
    this->runtime.last_item = 0;
    this->setup_buttons();
}

void TRIBE_Screen_Game::set_redraw(RedrawMode param_1) {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x004959A0
    TPanel::set_redraw(param_1);
}

void TRIBE_Screen_Game::set_overlapped_redraw(TPanel* param_1, TPanel* param_2, RedrawMode param_3) {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x004959B0
    (void)param_2;
...
long TRIBE_Screen_Game::handle_size(long param_1, long param_2) {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x00495AE0.
    ...
    if (rge_base_game != nullptr) {
        ...
        if (player != nullptr) {
            this->player_changed((int)player->id, (int)player->id);
        }
    }
    return 0;
}
...
void TRIBE_Screen_Game::set_map_buttons_redraw(RedrawMode param_1) {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x00495A90
    if (this->width() < 800) {
        if (this->runtime.button_panel[5] != nullptr) {
...
long TRIBE_Screen_Game::handle_user_command(uint param_1, long param_2) {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x00497280
...
void TRIBE_Screen_Game::stop_sound_system() {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049D4F0.
    if (this->runtime.game_over_sound != nullptr) {
        delete this->runtime.game_over_sound;
        this->runtime.game_over_sound = nullptr;
    }
...
int TRIBE_Screen_Game::restart_sound_system() {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049D530.
    int ok = 1;
...
long TRIBE_Screen_Game::wnd_proc(void* param_1, uint param_2, uint param_3, long param_4) {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x004970A0
...
long TRIBE_Screen_Game::handle_mouse_down(uchar param_1, long param_2, long param_3, int param_4, int param_5) {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x00497210
    for (int i = 0; i < 6; ++i) {
        if (this->runtime.message_panel[i] != nullptr) {
            this->runtime.message_panel[i]->remove_message();
        }
...
long TRIBE_Screen_Game::action(TPanel* param_1, long param_2, ulong param_3, ulong param_4) {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x004982E0.
    if (param_1 == nullptr) {
    ...
    if (panel_name != nullptr) {
        if (strcmp(panel_name, "RestartDialog") == 0) {
            ...
            if (param_2 == 0 && comm != nullptr) {
                scr_game_field_i32(this, 0x730) = 1;
...
void TRIBE_Screen_Game::show_timings(char* param_1, char* param_2) {
    ...
    (void)param_1;
    const char* next = (param_2 != nullptr) ? param_2 : "";
    char* current = this->runtime.fps_panel->get_text();
    if (current != nullptr && strcmp(current, next) == 0) {
        return;
    }
    this->runtime.fps_panel->set_text((char*)next);
}

void TRIBE_Screen_Game::show_comm(char* param_1, char* param_2, char* param_3, char* param_4, char* param_5, char* param_6) {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x00499520
    if (this->runtime.message_panel[0] != nullptr) this->runtime.message_panel[0]->show_message(TMessagePanel::DebugMessage, param_1, 0x56, 0, nullptr, 0, 0);
    if (this->runtime.message_panel[1] != nullptr) this->runtime.message_panel[1]->show_message(TMessagePanel::DebugMessage, param_2, 0x56, 0, nullptr, 0, 0);
    if (this->runtime.message_panel[2] != nullptr) this->runtime.message_panel[2]->show_message(TMessagePanel::DebugMessage, param_3, 0x56, 0, nullptr, 0, 0);
    if (this->runtime.message_panel[3] != nullptr) this->runtime.message_panel[3]->show_message(TMessagePanel::DebugMessage, param_4, 0x56, 0, nullptr, 0, 0);
    if (this->runtime.message_panel[4] != nullptr) this->runtime.message_panel[4]->show_message(TMessagePanel::DebugMessage, param_5, 0x56, 0, nullptr, 0, 0);
    if (this->runtime.message_panel[5] != nullptr) this->runtime.message_panel[5]->show_message(TMessagePanel::DebugMessage, param_6, 0x56, 0, nullptr, 0, 0);
}

void TRIBE_Screen_Game::show_ai(char* param_1, char* param_2, char* param_3, char* param_4, char* param_5, char* param_6) {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x004995D0
    if (this->runtime.message_panel[0] != nullptr) this->runtime.message_panel[0]->show_message(TMessagePanel::DebugMessage, param_1, 0x56, 0, nullptr, 0, 0);
    if (this->runtime.message_panel[1] != nullptr) this->runtime.message_panel[1]->show_message(TMessagePanel::DebugMessage, param_2, 0x56, 0, nullptr, 0, 0);
    if (this->runtime.message_panel[2] != nullptr) this->runtime.message_panel[2]->show_message(TMessagePanel::DebugMessage, param_3, 0x56, 0, nullptr, 0, 0);
    if (this->runtime.message_panel[3] != nullptr) this->runtime.message_panel[3]->show_message(TMessagePanel::DebugMessage, param_4, 0x56, 0, nullptr, 0, 0);
...
void TRIBE_Screen_Game::show_message(int type, char* text, unsigned char color1, unsigned char color2) {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x00499680.
...
void TRIBE_Screen_Game::disable_unused_buttons() {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049A8A0.
    for (int i = 0; i < 12; ++i) {
...
char* TRIBE_Screen_Game::calc_text_msg(char* param_1, Item_Avail* param_2, long param_3, long param_4) {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049A8F0.
    (void)param_1;
...
short TRIBE_Screen_Game::calc_button_loc(unsigned char param_1) {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049AB10.
...
void TRIBE_Screen_Game::set_button(
    ...
    char* param_11,
    int param_12) {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049AB60.
...
void TRIBE_Screen_Game::handle_anim_pal() {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049B830.
...
void TRIBE_Screen_Game::handle_terrain_sound() {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049B910.
...
void TRIBE_Screen_Game::handle_mouse_pointing_at() {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049BAF0.
...
void TRIBE_Screen_Game::command_add_attribute(int param_1) {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049BC60.
...
void TRIBE_Screen_Game::command_ai_info() {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049BCA0.
    for (int i = 0; i < 6; ++i) {
        if (this->runtime.message_panel[i] != nullptr) {
            this->runtime.message_panel[i]->remove_message();
        }
...
void TRIBE_Screen_Game::command_build() {
    ...
    rge_base_game->set_game_mode(6, 1);
    this->runtime.last_item = 0;
    this->setup_buttons();
}
...
void TRIBE_Screen_Game::command_building(int param_1) {
    ...
    if (player->check_obj_cost((short)param_1, &missing_attr, 1.0f, 0) != 0) {
        ...
        rge_base_game->set_game_mode(6, 0);
        this->setup_buttons();
        return;
    }
...
void TRIBE_Screen_Game::command_cancel_building() {
    ...
    TRIBE_Player* player = (TRIBE_Player*)rge_base_game->get_player();
    if (player != nullptr) {
        player->command_building_cancel();
    }
}
...
void TRIBE_Screen_Game::command_chat() {
    ...
    if (scr_game_field_i32(this, 0x7B4) == 0 && rge_base_game->prog_mode == 5) {
        if (this->runtime.message_panel[0] != nullptr) {
            char* text = this->get_string(0xBFE);
            this->runtime.message_panel[0]->show_message(TMessagePanel::BadMessage, text, '2', 0, nullptr, 0, 0);
        }
        return;
    }
    (void)new TribeSendMessageDialog((TScreenPanel*)this);
}
...
void TRIBE_Screen_Game::command_quick_chat() {
    ...
    (void)new TribeSendQuickMessageDialog((TScreenPanel*)this, TribeSendQuickMessageDialog::ToAll);
}
...
void TRIBE_Screen_Game::command_convert() {
    ...
    if (this->runtime.message_panel[0] != nullptr) {
        char* text = this->get_string(0xBE4);
        this->runtime.message_panel[0]->show_message(TMessagePanel::BadMessage, text, '2', 0, nullptr, 0, 0);
    }
    rge_base_game->set_game_mode(0x12, 0);
    rge_base_game->play_sound(0x65);
    this->setup_buttons();
}
...
void TRIBE_Screen_Game::command_diplomacy() {
    ...
    if (rge_base_game->singlePlayerGame() != 0) {
        rge_base_game->set_paused(1, 1);
    }
    (void)new TribeDiplomacyDialog((TScreenPanel*)this);
}
...
void TRIBE_Screen_Game::command_fps() {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049C280.
...
void TRIBE_Screen_Game::command_game_speed(int param_1) {
    ...
    const ulong now = debug_timeGetTime("C:\\msdev\\work\\age1_x1\\scr_game.c", 0x13FE);
    if ((now - scr_game_field_u32(this, 0x7BC) < 1000) && rge_base_game->singlePlayerGame() == 0) {
        return;
    }
    scr_game_field_u32(this, 0x7BC) = now;

    if (rge_base_game->prog_mode == 5) {
        return;
    }

    const float old_speed = this->runtime.world->game_speed;
    float new_speed = 0.0f;
    if (param_1 == 0) {
        if (old_speed == 16.0f) new_speed = 8.0f;
        else if (old_speed == 8.0f) new_speed = 4.0f;
        else if (old_speed == 4.0f) new_speed = 2.0f;
        else if (old_speed == 2.0f) new_speed = 1.5f;
        else if (old_speed == 1.5f) new_speed = 1.0f;
    } else if (old_speed == 1.0f) {
        new_speed = 1.5f;
    } else if (old_speed == 1.5f) {
        new_speed = 2.0f;
    } else if (rge_base_game->singlePlayerGame() != 0 && rge_base_game->gameDeveloperMode() != 0) {
        if (old_speed == 2.0f) new_speed = 4.0f;
        else if (old_speed == 4.0f) new_speed = 8.0f;
        else if (old_speed == 8.0f) new_speed = 16.0f;
    }

    if (new_speed != 0.0f) {
        if (rge_base_game->multiplayerGame() != 0 && comm != nullptr) {
            char msg[256];
            char* text = this->get_string(0xBF3);
            _snprintf(msg, sizeof(msg), "%s", text);
            msg[sizeof(msg) - 1] = '\0';
            ((TCommunications_Handler*)comm)->SendChatMsgAll(msg);
        }
        if (this->runtime.world->commands != nullptr) {
            ((TRIBE_Command*)this->runtime.world->commands)->command_game_speed(new_speed);
        }
        if (rge_base_game->singlePlayerGame() != 0) {
...
void TRIBE_Screen_Game::command_group() {
    ...
    TRIBE_Player* player = (TRIBE_Player*)rge_base_game->get_player();
    if (player != nullptr) {
        player->group_objects(-1);
    }
}

void TRIBE_Screen_Game::command_group_by_number(int param_1) {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049C570.
...
void TRIBE_Screen_Game::command_heal() {
    ...
    if (this->runtime.message_panel[0] != nullptr) {
        char* text = this->get_string(0xBE3);
        this->runtime.message_panel[0]->show_message(TMessagePanel::BadMessage, text, '2', 0, nullptr, 0, 0);
    }
    rge_base_game->set_game_mode(9, 0);
    rge_base_game->play_sound(0x66);
    this->setup_buttons();
}
...
void TRIBE_Screen_Game::command_repair() {
    ...
    if (this->runtime.message_panel[0] != nullptr) {
        char* text = this->get_string(0xBEC);
        this->runtime.message_panel[0]->show_message(TMessagePanel::BadMessage, text, '2', 0, nullptr, 0, 0);
    }
    rge_base_game->set_game_mode(10, 0);
    rge_base_game->play_sound(0x68);
    this->setup_buttons();
}

void TRIBE_Screen_Game::command_select_group(int param_1, int param_2) {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049C710.
    if (rge_base_game == nullptr || rge_base_game->get_paused() != 0) {
        return;
    }
...
void TRIBE_Screen_Game::command_menu() {
    ...
    if (rge_base_game->prog_mode == 6 && this->runtime.world != nullptr && this->runtime.world->commands != nullptr) {
        this->runtime.world->commands->do_commands();
    }
    if (rge_base_game->singlePlayerGame() != 0) {
        rge_base_game->set_paused(1, 1);
    }
    (void)new TribeMenuDialog((TScreenPanel*)this);
}

void TRIBE_Screen_Game::command_more() {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049C800.
    if (rge_base_game == nullptr || rge_base_game->get_paused() != 0) {
        return;
    }
...
void TRIBE_Screen_Game::command_outline() {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049C920.
    if (rge_base_game == nullptr) {
        return;
    }
...
void TRIBE_Screen_Game::command_quit() {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049CAC0.
    if (comm != nullptr) {
        ((TCommunications_Handler*)comm)->SendIResignMsg();
...
void TRIBE_Screen_Game::command_ungroup() {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049CFB0.
...
void TRIBE_Screen_Game::command_unselect() {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049D040.
    if (rge_base_game == nullptr || rge_base_game->get_paused() != 0) {
        return;
    }
...
void TRIBE_Screen_Game::command_view_selected() {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049D090.
...
void TRIBE_Screen_Game::command_view_tribe() {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049D100.
...
void TRIBE_Screen_Game::command_visibility() {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049D1B0.
    if (rge_base_game != nullptr && rge_base_game->allowCheatCodes() != 0) {
...
void TRIBE_Screen_Game::command_formation(int param_1) {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049D290.
    if (rge_base_game == nullptr || rge_base_game->get_paused() != 0) {
        return;
    }
    TRIBE_Player* player = (TRIBE_Player*)rge_base_game->get_player();
...
void TRIBE_Screen_Game::command_trade_attribute(long param_1) {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049D340.
    if (rge_base_game == nullptr || rge_base_game->get_paused() != 0) {
        return;
    }
    TRIBE_Player* player = (TRIBE_Player*)rge_base_game->get_player();
...
void TRIBE_Screen_Game::command_tab_selected(int param_1) {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049D370.
    if (rge_base_game == nullptr || rge_base_game->get_paused() != 0) {
        return;
    }
    TRIBE_Player* player = (TRIBE_Player*)rge_base_game->get_player();
...
int TRIBE_Screen_Game::any_selected_have_action() {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049D440.
    if (rge_base_game == nullptr) {
        return 0;
    }
...
void TRIBE_Screen_Game::add_log(char* text) {
    // Fully verified. Source of truth: scr_game.cpp.decomp @ 0x0049D540.
