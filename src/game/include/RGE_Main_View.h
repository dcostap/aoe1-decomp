...
public:
    ...
    virtual int command_place_object(long param_1, long param_2, int param_3); // vt[63] (0xFC)
    virtual int command_make_do(long param_1, long param_2, int param_3, short param_4); // vt[64] (0x100)
    virtual int command_make_move(long param_1, long param_2); // vt[65] (0x104)
    ...
    long mouse_last_x;
    long mouse_last_y;
    unsigned long last_key_scroll_time;
    int key_scrolling;
    RGE_Player* save_player;
};
static_assert(sizeof(RGE_Main_View) == 0x380, "Size mismatch");
